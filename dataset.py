...
class MergeDataset(Dataset):
    ...
    def __len__(self):
        # 可以返回任意一个数据集的长度，或者 None，但 Dataset 要求必须有整数长度
        # 通常取最长或最短的数据集长度，这里我们取第一个数据集的长度作为参考
        if not self._dataset_list:
            return 0
        return len(self._dataset_list[0])  # 或者 max(len(d) for d in self._dataset_list)

    def __getitem__(self, index):
        data_list = []
        for dataset in self._dataset_list:
            # 每次随机从当前 dataset 中采样一个 item
...
def hdr_collate_fn(batch):
    """
    batch: list of tuples (hdr, crf, invcrf, t)
    returns:
        hdr_tensor: [B, C, H, W]
        crf_tensor: [B, S]
        invcrf_tensor: [B, S]
        t_tensor:   [B]
    """
    hdr_list, crf_list, invcrf_list, t_list = zip(*batch)

    # Convert to Tensor and permute NHWC -> NCHW
    hdr_tensor = torch.from_numpy(np.stack(hdr_list)).float()
    crf_tensor = torch.from_numpy(np.stack(crf_list)).float()
    invcrf_tensor = torch.from_numpy(np.stack(invcrf_list)).float()
    t_tensor = torch.tensor(t_list).float()

    return hdr_tensor, crf_tensor, invcrf_tensor, t_tensor